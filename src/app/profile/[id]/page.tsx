...
export default async function ProfilePage(props: PageProps) {
  ...
  return (
    <div className="flex flex-col mx-auto container py-10 gap-15 px-8 ">
      <div className="w-full flex flex-col md:flex-row justify-between DEBUG">
        <div className="flex w-full md:w-fit">
          <div className="flex flex-col items-center">
            <Avatar className="size-16 md:size-40 lg:size-64">
            ...
            <Button variant={"secondary"} className="md:hidden w-fit">
              Editar
            </Button>
          </div>
          ...
          <div className="flex flex-col px-2 flex-1">
            <div className="flex flex-col md:flex-row text-xl md:text-2xl xl:text-3xl">
              ...
              <p className="font-normal overflow-hidden text-ellipsis">@{id}</p>
            </div>
      ...
      {/* <div>
        <Card className="w-full flex flex-row p-3">
          <Avatar className="size-16 md:size-40 lg:size-16">
            <AvatarImage src="https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png" />
            <AvatarFallback>CN</AvatarFallback>
          </Avatar>
          <div className="w-full flex flex-col justify-center">
            <CardHeader className="p-0">
              <div className="flex flex-row space-x-2 truncate">
                ...
                <CardTitle className="font-normal text-3xl overflow-hidden text-ellipsis">
                  @{id}
                </CardTitle>
              </div>
              <CardDescription className="font-extralight text-xl italic">
                Miembro desde: 31/05/2025
              </CardDescription>
          ...
          <ScrollArea className="h-40">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((item) => (
                <div
                  key={item}
                  className="p-2 flex flex-row bg-primary-foreground items-center h-18 border border-border rounded-sm"
                >
            ...
            <ScrollArea className="h-48 p-2">
              <div className="space-y-3">
                {[1, 2, 3, 4].map((item) => (
                  <Card
                    key={item}
                    className="p-3 flex border borde-border bg-primary-foreground"
                  >
